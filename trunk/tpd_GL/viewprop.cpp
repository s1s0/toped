...
{
   setUU(1);
   // first build-in colours ...
   _drawprop->addColor("zoomW", 179, 179, 179, 128);
   // ... then build-in patterns ...
   byte* dfltFill = DEBUG_NEW byte[128];
   std::fill(dfltFill, dfltFill + 128, 0xFF);
   _drawprop->addFill("zoomW", dfltFill);
   // ... then build-in stipple lines ...
   _drawprop->addLine("zoomW","zoomW", 0x5555,1,1);
   // ... and finally build-in layers
   _drawprop->addLayer("zoomW", ZOM_LAY_DEF,"zoomW", "zoomW", "zoomW");
   //
...
{
   _drawprop = propDB;
   _drawprop->setState(layprop::prsDB);

            ...
            struct libavl_allocator *allocator)
{
  ...
  tree = static_cast<avl_table*>(allocator->libavl_malloc (allocator, sizeof *tree));
  if (tree == NULL)
    return NULL;
...
avl_probe (struct avl_table *tree, void *item)
{
  ...
  n = q->avl_link[dir] =
    static_cast<avl_node*>(tree->avl_alloc->libavl_malloc (tree->avl_alloc, sizeof *n));
  if (n == NULL)
    return NULL;
...
void *
avl_t_replace (struct avl_traverser *trav, void *newp)
{
  void *old;

  assert (trav != NULL && trav->avl_node != NULL && newp != NULL);
  old = trav->avl_node->avl_data;
  trav->avl_node->avl_data = newp;
  return old;
}

static void
copy_error_recovery (struct avl_node **stack, int height,
                     struct avl_table *newp, avl_item_func *destroy)
{
  assert (stack != NULL && height >= 0 && newp != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->avl_link[1] = NULL;
  avl_destroy (newp, destroy);
}
          ...
          avl_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct avl_table *newp;
  const struct avl_node *x;
  struct avl_node *y;

  assert (org != NULL);
  newp = avl_create (org->avl_compare, org->avl_param,
                    allocator != NULL ? allocator : org->avl_alloc);
  if (newp == NULL)
    return NULL;
  newp->avl_count = org->avl_count;
  if (newp->avl_count == 0)
    return newp;

  x = (const struct avl_node *) &org->avl_root;
  y = (struct avl_node *) &newp->avl_root;
  for (;;)
    {
      while (x->avl_link[0] != NULL)
        {
          ...
          y->avl_link[0] =
            static_cast<avl_node*>(newp->avl_alloc->libavl_malloc (newp->avl_alloc,
                                           sizeof *y->avl_link[0]));
          if (y->avl_link[0] == NULL)
            {
              if (y != (struct avl_node *) &newp->avl_root)
                {
                  y->avl_data = NULL;
                  y->avl_link[1] = NULL;
                }

              copy_error_recovery (stack, height, newp, destroy);
              return NULL;
            }

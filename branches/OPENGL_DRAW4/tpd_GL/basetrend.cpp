...
unsigned trend::TenderSWire::ssize()
{
   ...
   // the rules when the selected objects were fitted into the whole Collect
   // class hierarchy.
   if (!_celno)
   {
...
// class TenderTV
//
trend::TenderTV::TenderTV(TenderRef* const refCell, bool filled, bool reusable,
...
void trend::TenderTV::registerBox (TenderCnvx* cobj)
{
...
void trend::TenderTV::registerPoly (TenderNcvx* cobj, const TessellPoly* tchain)
{
   unsigned allpoints = cobj->csize();
...
void trend::TenderTV::registerWire (TenderWire* cobj)
{
   unsigned allpoints = cobj->csize();
...
void trend::TenderTV::registerText (TenderText* cobj, TextOvlBox* oobj)
{
...
unsigned trend::TenderTV::num_total_points()
{
...
unsigned trend::TenderTV::num_total_indexs()
{
...
//}
//
//void trend::TenderTV::collect(TNDR_GLDATAT* point_array, unsigned int* index_array, unsigned int*)
//{
...
//}
//
//trend::TenderRef* trend::TenderTV::swapRefCells(TenderRef* newRefCell)
//{
...
// class TenderLay
//
trend::TenderLay::TenderLay(): _cslice(NULL),
 ...
 */
void trend::TenderLay::newSlice(TenderRef* const ctrans, bool fill, bool reusable, unsigned slctd_array_offset)
{
...
void trend::TenderLay::newSlice(TenderRef* const ctrans, bool fill, bool reusable)
{
   _cslice = DEBUG_NEW TenderTV(ctrans, fill, reusable, 2 * _num_total_points, _num_total_indexs);
}

bool trend::TenderLay::chunkExists(TenderRef* const ctrans, bool filled)
{
...
void trend::TenderLay::box  (const int4b* pdata)
{
   _cslice->registerBox(DEBUG_NEW TenderBox(pdata));
}

void trend::TenderLay::box (const int4b* pdata, const SGBitSet* ss)
{
...
void trend::TenderLay::poly (const int4b* pdata, unsigned psize, const TessellPoly* tpoly)
{
   _cslice->registerPoly(DEBUG_NEW TenderNcvx(pdata, psize), tpoly);
}

void trend::TenderLay::poly (const int4b* pdata, unsigned psize, const TessellPoly* tpoly, const SGBitSet* ss)
{
...
void trend::TenderLay::wire (int4b* pdata, unsigned psize, WireWidth width, bool center_only)
{
   _cslice->registerWire(DEBUG_NEW TenderWire(pdata, psize, width, center_only));
}

void trend::TenderLay::wire (int4b* pdata, unsigned psize, WireWidth width, bool center_only, const SGBitSet* ss)
{
...
void trend::TenderLay::text (const std::string* txt, const CTM& ftmtrx, const DBbox* ovl, const TP& cor, bool sel)
{
...
void trend::TenderLay::registerSBox (TenderSBox* sobj)
{
...
void trend::TenderLay::registerSOBox (TextSOvlBox* sobj)
{
   _slct_data.push_back(sobj);
   _asindxs[llps] += 4;
   _asobjix[llps]++;
...
void trend::TenderLay::registerSPoly (TenderSNcvx* sobj)
{
   _slct_data.push_back(sobj);
   if ( sobj->partSelected() )
   {
...
void trend::TenderLay::registerSWire (TenderSWire* sobj)
{
   _slct_data.push_back(sobj);
   if ( sobj->partSelected() )
   {
...
unsigned trend::TenderLay::total_slctdx()
{
...
void trend::TenderRefLay::addCellOBox(TenderRef* cRefBox, word alphaDepth, bool selected)
{
   if (selected)
   {
...
unsigned trend::TenderRefLay::total_points()
{
   return (_alvrtxs + _asindxs);
}

unsigned trend::TenderRefLay::total_indexes()
{
...
// class Collect
//
trend::Collect::Collect( layprop::DrawProperties* drawprop, real UU ) :
...
bool trend::Collect::chunkExists(const LayerDef& laydef, bool has_selected)
{
...
void trend::Collect::setLayer(const LayerDef& laydef, bool has_selected)
{
   // Reference layer is processed differently (pushCell), so make sure
   // that we haven't got here with REF_LAY by accident
   assert(REF_LAY_DEF != laydef);
...
void trend::Collect::pushCell(std::string cname, const CTM& trans, const DBbox& overlap, bool active, bool selected)
{
...
void trend::Collect::grcpoly(int4b* pdata, unsigned psize)
{
   assert(_grcLayer);
   _grcLayer->poly(pdata, psize, NULL);
}

void trend::Collect::wire (int4b* pdata, unsigned psize, WireWidth width)
{
...
void trend::Collect::wire (int4b* pdata, unsigned psize, WireWidth width, const SGBitSet* psel)
{
   // first check whether to draw only the center line
   DBbox wsquare = DBbox(TP(0,0),TP(width,width));
   bool center_line_only = !wsquare.visible(topCTM() * scrCTM(), visualLimit());
...
void trend::Collect::grcwire (int4b* pdata, unsigned psize, WireWidth width)
{
   // first check whether to draw only the center line
   DBbox wsquare = DBbox(TP(0,0),TP(width,width));
   bool center_line_only = !wsquare.visible(topCTM() * scrCTM(), visualLimit());
...
void trend::Collect::arefOBox(std::string cname, const CTM& trans, const DBbox& overlap, bool selected)
{
   if (selected || (!_drawprop->isCellBoxHidden()))
   {
      TenderRef* cRefBox = DEBUG_NEW TenderRef(cname,
...
void trend::Collect::text (const std::string* txt, const CTM& ftmtrx, const DBbox& ovl, const TP& cor, bool sel)
{
   if (sel)
...
void trend::Collect::grid(const real step, const std::string color)
{
...
void trend::Collect::setGrcLayer(bool setEData, const LayerDef& laydef)
{
   if (setEData)
   {
...
bool trend::Collect::preCheckCRS(const laydata::TdtCellRef* ref, layprop::CellRefChainType& crchain)
{

...
/*   if (!(_cifFh = fopen(fname.c_str(),"wt")))
   ...
   _file << "( CIF generated by : Toped 0.9.x );"      << std::endl;
...
void CIFin::CifExportFile::definitionStart(std::string name)
{
   ...
   if (_verbose)
      _file << "Definition Start #" << _lastcellnum << ";"<< std::endl;
   else
      _file << "DS " << _lastcellnum << ";" << std::endl;
...
void CIFin::CifExportFile::box(unsigned length, unsigned width, TP& center)
{
   if (_verbose)
